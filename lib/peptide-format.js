...
/** "early-human" -> "Early Human", "commercial_blend" -> "Commercial Blend" */
...
export const stackTypeColor = (v) => STACK_TYPE_COLOR[v] || '#6b7788'
...
/** Match a stack against a free-text query across name, category, aliases. */
export function matchesStackQuery(stack, query) {
  const q = query.trim().toLowerCase()
  if (!q) return true
  const haystack = [stack.name, stack.category, ...(stack.aliases || [])]
  return haystack.some((v) => v && String(v).toLowerCase().includes(q))
}
